...
public class HW_4_3 {
    public static void main(String[] args) {
        ...
        System.out.println(
                "Калькулятор для выполнения операций + , - , * " +
                        "или /\nПри вводе 'Отмена' последнее действие отменяется и возвращается предыдущий результат");
        ...
        while (calc) {
            ...
            if (operation.equals("отмена")) {
                if (!number_list.isEmpty()) {
                    rollback_operation(number_list);
    ...
    public static double input_number(Scanner scanner) {
        ...
        try {
            ...
            System.out.println("Введенные символы на являются числом. Введите число");
            scanner.nextLine();
    ...
    public static void rollback_operation(Deque<Double> list) {
        if (!list.isEmpty()) {
            list.removeLast();
